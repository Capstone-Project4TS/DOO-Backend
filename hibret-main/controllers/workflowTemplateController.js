...
export const createWorkflowTemplate = async (req, res) => {
  try {
    ...
    const existingTemplate = await WorkflowTemplate.findOne({ name });
    if (existingTemplate) {
...
export async function updateWorkflowTemplate(req, res) {
  try {
    ...
    // Check if the provided name is different from the existing name
    if (existingWorkflowTemplate.name !== name) {
      return res.status(400).json({ error: 'Cannot update workflow template name' });
    }
